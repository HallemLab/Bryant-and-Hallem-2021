...
if (!file.exists(c("../Data/tidy_genomic_CAI.csv"))) {
  ...
  dat.genomic.cleaned <- lapply(seq_along(dat.genomic), function(x){
    ...
    dat.genomic[[x]] %>%
      ...
      dplyr::mutate(Sr_CAI = if("Sr_CAI" %in% colnames(.)) Sr_CAI else NA) %>%
      dplyr::mutate(Ce_CAI = if("Ce_CAI" %in% colnames(.)) Ce_CAI else NA) %>%
...
### Kruskal-Wallis Test with Dunn's post-hoc tests for differences in genomic CAI values across genus groups  
...
### Kruskal-Wallis Test with Dunn's Post tests for differences in genomic GC values across genus groups  
```{r genomic.GC.kwtest2}
